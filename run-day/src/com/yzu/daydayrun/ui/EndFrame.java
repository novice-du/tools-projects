...
public class EndFrame extends JFrame implements MouseListener{
	...
	public EndFrame(Person p){
		...
		lblAgain =  new JLabel(new ImageIcon("F:/PersonalProject/run-day/run-day/image/again.png"));
		lblAgain.addMouseListener(this);
		lblAgain.setBounds(260, 225, 60, 25);
		
		lblMain = new JLabel(new ImageIcon("F:/PersonalProject/run-day/run-day/image/endMain.png"));
		...
		this.setIconImage(new ImageIcon("F:/PersonalProject/run-day/run-day/image/end_1.png").getImage());
...
class BackEnd extends JPanel{
	...
	public BackEnd(Person p,List<User> users){
		...
		this.users = users;
		try {
			img = ImageIO.read(new File("F:/PersonalProject/run-day/run-day/image/end.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
